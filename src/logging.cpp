...
#define UTIL_CONTEXT util::logging::log_context(__FILE__, __LINE__, __FUNCTION__)

// Stub is needed since pthreads
// don't know about C++ objects
extern "C" void* logproxy_run(void* p) {
	auto data = static_cast<util::logging::logproxy::threadlocal*>(p);
	data->proxy->run(data);
	return 0;
}
...
namespace {
	...
	void syslog_write(int priority, const char* str, va_list arglist) {
		if (!_syslog_open) {
			openlog("util", LOG_CONS, LOG_USER);
			_syslog_open = true;
		}
	...
	namespace logging {
		...
		void logproxy::run(logproxy::threadlocal* data) {
			...
			for (;;) {
				...
				if (amt <= 0) {
					if (amt < 0) {
						UTIL_CONTEXT.error("Error in read() on fd %d. %s", data->outpipe, strerror(errno));
					}
					break;
				}
		...
		bool logproxy::create() {
			...
			if (pipe(pip) == -1) {
				UTIL_CONTEXT.error("Failed to create pipe for subprocess logging.");
				return false;
			}
